...
function _onLogoutComplete (e) {
    _client = Titanium.Network.createHTTPClient({
        onload: function (e) {
            Ti.App.fireEvent(app.loginEvents['LOGOUT_COMPLETE'], e);
...
function _onLoginComplete (e) {
    ...
    if (_failureRegex.exec(_client.responseText)) {
        ...
        Ti.App.fireEvent(app.loginEvents['LOGIN_METHOD_COMPLETE'], e);
    }
};
...
function _onInitialResponse (e) {
    ...
    if (_client.responseText.indexOf('"layout": [') > -1) return Ti.App.fireEvent(app.loginEvents['LOGIN_METHOD_COMPLETE'], e);
